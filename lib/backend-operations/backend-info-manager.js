...
function updateAWSExportFile(projectInfo, awsDetails, callback){ 
    awsExportFileManager.getAWSExportFile(projectInfo, awsDetails, (configurationFileName)=>{
        insertAppSyncJSIntoAWSExportJSFile(projectInfo.ProjectPath, configurationFileName)
        ...
        if(srcDir && fs.existsSync(srcDir)){
            ...
            console.log('    ' + chalk.blue(pathManager.getSrcDirExportFilePath_relative(projectInfo, configurationFileName)))    
        }
        if(callback){
            callback()
        }
...
function insertAppSyncJSIntoAWSExportJSFile(projectPath, configurationFileName){
    let appsyncJSObj = opsAppSync.getAppSyncJS(projectPath)
    if(appsyncJSObj && Object.keys(appsyncJSObj).length > 0){
        let awsExportFilePath = pathManager.getAWSExportFilePath(projectPath, configurationFileName)
        if(configurationFileName && /\.json$/.test(configurationFileName)){
            let keys = Object.keys(appsyncJSObj)
            keys.forEach(key => {
                let value = appsyncJSObj[key]
                let newKey = key.charAt(0).toUpperCase() + key.slice(1)
                delete appsyncJSObj[key]
                appsyncJSObj[newKey] = value
            })

            let configObj = dfOps.readJsonFile(awsExportFilePath)
            configObj["AppSync"] = {
                "Default": appsyncJSObj
            }
            dfOps.writeJsonFile(awsExportFilePath, configObj)
        }else{
