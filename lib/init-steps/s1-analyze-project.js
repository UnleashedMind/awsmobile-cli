...
function guessFramework(initInfo){
    ...
    }else{//it could be native app
        if(isAndroidProject(initInfo)){
            frameWorkName = 'android'
        }else if(isXCodeProject(initInfo)){
            frameWorkName = 'xcode'
        }
    }
...
function isAndroidProject(initInfo){
    let result = false
    let settingsFilePath = path.join(initInfo.projectPath, 'settings.gradle')
    let buildFilePath = path.join(initInfo.projectPath, 'build.gradle')
...
function isXCodeProject(initInfo){
    let result = false
    
    let dirs = fs.readdirSync(initInfo.projectPath)
    dirs.forEach(function(dir) {
        if (fs.statSync(path.join(initInfo.projectPath, dir)).isDirectory()) {
...
build.gradle
settings.gradle
in build.gradle the buildscript there's android dependencies
/app folder
existence of /app//src/androidTest folder. 

/app/src/main/androidManifest.xml most definitive check. 
*/