...
const frameworkConfigMappings = require('../utils/framework-config-mapping.js')
...
function configureProject(initInfo){
    ...
    if(initInfo.framework){
        ...
        if(initInfo.framework == 'xcode'){
            config.SourceDir = initInfo.srcDir
        }
    }

    if(initInfo.yesFlag){
        initInfo.projectConfig = config
        persistProjectConfig(initInfo)
        return initInfo
    }else{
