...
function getAWSExportFilePath(projectPath, awsConfigurationFileName)
{
    let fileName = awsConfigurationFileName ? awsConfigurationFileName : awsmobilejsConstant.AWSExportFileName
    return path.normalize(path.join(getCurrentBackendInfoDirPath(projectPath), fileName))
}
...
function getAWSExportFilePath_relative(projectPath, awsConfigurationFileName)
{
    let fullPath = getAWSExportFilePath(projectPath, awsConfigurationFileName)
    return path.relative(projectPath, fullPath)
}
...
function getAppSyncInfoFilePath(projectPath)
{
    return path.normalize(path.join(getInfoDirPath(projectPath), awsmobilejsConstant.AppSyncInfoFileName))
}
...
function getSrcDirExportFilePath(projectInfo, configurationFileName)
{
    let filePathInSrcDir
    if(projectInfo.SourceDir){
        let srcDirPath = path.normalize(path.join(projectInfo.ProjectPath, projectInfo.SourceDir))
        if(projectInfo.Framework && projectInfo.Framework == 'android'){
            srcDirPath = path.normalize(path.join(srcDirPath, 'main/res/raw'))
            fs.ensureDirSync(srcDirPath)
        }
        if(!configurationFileName){
            configurationFileName = awsmobilejsConstant.AWSExportFileName
        }
        filePathInSrcDir = path.normalize(path.join(srcDirPath, configurationFileName))
    }
    return filePathInSrcDir
}

function getSrcDirExportFilePath_relative(projectInfo, configurationFileName)
{
    let relative_path
    let fullPath = getSrcDirExportFilePath(projectInfo, configurationFileName)
    if(fullPath){
