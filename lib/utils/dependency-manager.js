...
function isJSProject(initInfo){
    let result = true
    if(initInfo['framework'] && initInfo['framework'].length > 0)
    {
        let nativeFrameworks = ['android', 'objective-c', 'swift']
        if(nativeFrameworks.includes(initInfo['framework'])){
            result = false
        }
    }
    return result
